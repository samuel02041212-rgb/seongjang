...
export function KakaoRegisterForm({
  ...
  preview: { name: string | null; image: string | null };
  initialGender?: "" | "M" | "F";
  ...
  const parsedBirth = useMemo(() => {
    if (!initialBirthDate) return { y: "", m: "", d: "" };
    const dt = new Date(`${initialBirthDate}T12:00:00`);
  ...
  return (
    <div className="w-full max-w-md rounded-lg border border-line bg-surface p-6 shadow-sm sm:p-8">
      <h1 className="font-display text-xl text-ink">추가 정보 입력</h1>
      <p className="mt-1 text-sm text-muted">
        카카오 프로필을 확인하고 아래 정보를 입력해 주세요.
      </p>

      <div className="mt-5 flex items-center gap-3 rounded-lg border border-line/70 bg-bg px-3 py-3">
        <div className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-full border border-line bg-surface">
          {preview.image ? (
            ...
            <span className="text-sm text-muted">
              {(preview.name ?? "회원").charAt(0)}
            </span>
          )}
        </div>
        <p className="font-medium text-ink">{preview.name ?? "회원"}</p>
      </div>
