...
- **Commands:** `npm install`, `npm run dev`. DB: `npx prisma migrate dev`, `npx prisma generate`, `npx tsx prisma/seed.ts`. Destructive: `npm run db:reset-admin-only` — deletes all users/posts/etc. and leaves only admin `admin@seongjang.local` with login `a`/`a` (uses `DATABASE_URL`).
